...
export async function updateLeadStatus(
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to update lead status: ${response.status} ${response.statusText} - ${responseText}`
      );
    }
...
export async function updateLeadCustomFields(
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to update lead custom fields: ${response.status} ${response.statusText} - ${responseText}`
      );
    }
...
export async function updateLeadName(
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to update lead name: ${response.status} ${response.statusText} - ${responseText}`
      );
    }
...
export async function getLeadInfo(
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to get lead info: ${response.status} ${response.statusText} - ${responseText}`
      );
    }
...
export async function getContactInfo(
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to get contact info: ${response.status} ${response.statusText} - ${responseText}`
      );
    }
...
export async function getCurrentLeadStatus(
  leadId: string,
  config: KommoApiConfig
): Promise<keyof typeof STATUS_MAPPING | null> {
  try {
...
export async function registerUser(
  ...
  const platformConfig = USER_REGISTRATION_CONFIG.platform.find(
    (p) => p.name === platform
  );
  if (!platformConfig) {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to register user: ${response.status} ${response.statusText} - ${responseText}`
      );
    }
...
export async function registerUserWithRetry(
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      console.log(
        `🔐 Attempting user registration (attempt ${attempt + 1}/${
          maxRetries + 1
        }) with username: ${currentUsername} for platform: ${platform}`
      );
      const result = await registerUser(currentUsername, platform);
      console.log(
        `✅ User registration successful on attempt ${
          attempt + 1
      ...
      console.warn(
        `❌ User registration failed on attempt ${
          attempt + 1
        } for username: ${currentUsername}`,
        error
      );
      ...
      if (attempt < maxRetries) {
        ...
        const truncatedBase =
          currentUsername.length > maxBaseLength
        ...
        console.error(
          `❌ All registration attempts failed for base username: ${baseUsername}`
        );
        throw error;
      }
    }
...
interface ParamsRegisterMoneyMaker {
  phone: string;
  ref: string;
  name: string;
}
...
export async function registerMoneyMaker(
  ...
  try {
    ...
    logHttpError("registerMoneyMaker", error, url);
    throw error;
  }
}

// Complete user creation process
export async function createUserFromLead(
  leadId: string,
  config: KommoApiConfig,
  platform: "greenBet" | "moneyMaker"
): Promise<any> {
  try {
    ...
    // 2. Get main contact
    const mainContact =
      contacts.find((contact: any) => contact.is_main) || contacts[0];
    ...
    let phone = "";
    const customFields = contactInfo.custom_fields_values || [];
    ...
    try {
      if (platform === "greenBet") {
        // 4. Generate username
        username = generateUsername(contactInfo.name, contactInfo.id);
        console.log(`👤 Generated username: ${username}`);

        // 5. Register user in external API with retry logic
        console.log(
          `🔐 Registering user ${username} with retry logic for platform: ${platform}`
        );
